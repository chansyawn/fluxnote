...
pub fn save_window_position(app: &AppHandle, window_label: &str) -> tauri::Result<()> {
    let window = app
...
pub fn show_on_cursor_monitor(app: &AppHandle, window_label: &str) -> tauri::Result<()> {
    let window = app
        .get_webview_window(window_label)
        .ok_or_else(|| tauri::Error::WindowNotFound)?;
    ...
    let monitor = monitors
        ...
        .ok_or_else(|| tauri::Error::Anyhow(anyhow::anyhow!("Failed to find monitor at cursor position")))?;
