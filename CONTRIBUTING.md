...
When releasable changes land, release-please opens a release pull request that updates the desktop app version and `CHANGELOG.md`. Merging that pull request creates a draft GitHub Release, builds the signed app artifacts, uploads them, and then publishes the release.
