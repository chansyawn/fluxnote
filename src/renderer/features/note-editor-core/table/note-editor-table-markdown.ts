...
import { NOTE_EDITOR_IMAGE_TRANSFORMER } from "@renderer/features/note-editor-core/image/note-editor-image-markdown";
...
import {
  ...
  isMarkdownTableRow,
  splitTableRow,
} from "./note-editor-table-utils";

const TABLE_ROW_REGEXP = /^(?:\|)(.+)(?:\|)\s?$/;

const TABLE_CELL_MARKDOWN_TRANSFORMERS = [
...
export const TABLE_TRANSFORMER: MultilineElementTransformer = {
  ...
  handleImportAfterStartMatch: ({ lines, rootNode, startLineIndex }) => {
    ...
    if (!dividerLine || !isMarkdownTableDividerRow(dividerLine)) {
      return null;
    }
    ...
    while (currentLineIndex < lines.length && isMarkdownTableRow(lines[currentLineIndex])) {
