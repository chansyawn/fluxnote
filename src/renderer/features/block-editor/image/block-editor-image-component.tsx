...
import { $moveCaretAfterImage, $moveCaretBeforeImage } from "./block-editor-image-cursor";
import { $isBlockEditorImageNode } from "./block-editor-image-node";
import {
  isInternalAssetUrl,
  resolveAssetUrl,
  warmImageDataUrlCache,
} from "./block-editor-image-utils";

interface BlockEditorImageComponentProps {
  altText: string;
  blockId: string;
  nodeKey: NodeKey;
  src: string;
...
export function BlockEditorImageComponent({
  altText,
  blockId,
  nodeKey,
  src,
}: BlockEditorImageComponentProps) {
    ...
    return mergeRegister(
      editor.registerCommand(
        ...
        (event) => {
          ...
          if (!target.closest(`[data-block-editor-image-key="${nodeKey}"]`)) {
            return false;
          }
  ...
  if (hasError) {
    return (
      <span
        className="block-editor__image block-editor__image--error"
        data-block-editor-image-key={nodeKey}
      >
        {i18n._({
  ...
  return (
    <span
      className="block-editor__image-shell"
      data-block-editor-image-key={nodeKey}
      ...
      tabIndex={0}
    >
      <img alt={altText} className="block-editor__image" draggable={false} src={resolvedSrc} />
    </span>
  );
}
