...
import { useBlockEditorBlockId } from "@renderer/features/block-editor/composer/block-editor-context";
import {
  $insertNodes,
  $isElementNode,
  $createNodeSelection,
...
export function BlockEditorImagePlugin() {
  const [editor] = useLexicalComposerContext();
  const blockId = useBlockEditorBlockId();
    ...
    return editor.registerCommand<ClipboardEvent>(
      ...
      (event) => {
        ...
        if (lexicalPayload && hasSerializedImageNodes(lexicalPayload.nodes)) {
          ...
          let pastedImageNodes: BlockEditorImageNode[] = [];
          editor.update(() => {
  ...
  useEffect(() => {
    return editor.registerNodeTransform(BlockEditorImageNode, (node) => {
      if ($isBlockEditorImageNode(node) && !node.getBlockId()) {
...
function hasSerializedImageNodes(nodes: readonly SerializedClipboardNode[]): boolean {
  return nodes.some(
    (node) =>
      node.type === "block-editor-image" ||
      (node.children ? hasSerializedImageNodes(node.children) : false),
  );
}

function collectImageNodes(nodes: readonly LexicalNode[]): BlockEditorImageNode[] {
  const imageNodes: BlockEditorImageNode[] = [];
...
async function rehomePastedImageNodes(
  ...
  imageNodes: readonly BlockEditorImageNode[],
