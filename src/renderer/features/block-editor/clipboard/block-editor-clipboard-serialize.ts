...
import { BLOCK_EDITOR_NODES } from "../composer/block-editor-nodes";
import { $isBlockEditorImageNode } from "../image/block-editor-image-node";
import {
  getCachedImageDataUrl,
  isInternalAssetUrl,
  loadImageClipboardPayload,
} from "../image/block-editor-image-utils";
import { BLOCK_EDITOR_MARKDOWN_TRANSFORMERS } from "../markdown/block-editor-markdown";
import type {
  ClipboardImageReference,
  ClipboardPayload,
  ClipboardSnapshot,
} from "./block-editor-clipboard-types";
  ...
  tempEditor.update(
    () => {
      ...
      payload = {
        ...
        imageRefs,
        lexicalJson,
        markdown: $convertToMarkdownString(BLOCK_EDITOR_MARKDOWN_TRANSFORMERS, root),
        singleSelectedImage: snapshot.singleSelectedImage,
      };
    },
    { discrete: true },
