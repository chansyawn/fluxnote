...
import type { TableOperationAction, TableStructureOperation } from "./table-operations";
  ...
  return (
    <DropdownMenu open={open} onOpenChange={(nextOpen) => onOpenChange(kind, nextOpen)}>
      <DropdownMenuTrigger
        ...
        className={cn(
          "absolute flex h-3 w-4.5 -translate-x-1/2 -translate-y-1/2 items-center justify-center bg-muted",
          kind === "row" && "rotate-90",
        )}
        data-table-control
        onPointerDown={(event) => {
