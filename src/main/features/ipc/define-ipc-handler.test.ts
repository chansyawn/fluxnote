...
import { defineIpcHandler } from "@main/features/ipc/define-ipc-handler";

describe("defineIpcHandler", () => {
  beforeEach(() => {
  ...
  it("parses requests and returns successful responses for trusted senders", async () => {
    ...
    defineIpcHandler<"sampleGreet">({
      command: ipcCommandContracts.sampleGreet,
      getTrustedWebContents: () => trustedSender as never,
      handler: async (request) => ({ message: `Hello ${request.name}` }),
    });
